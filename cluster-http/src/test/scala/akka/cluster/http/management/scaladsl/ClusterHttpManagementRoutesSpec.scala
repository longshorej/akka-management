  ...
  "Http Cluster Management Routes" should {
    ...
    "return shard type names" when {
      "calling GET /cluster/shards" in {
        ...
        val unmarshaledGetShardDetails = Unmarshal(responseGetShardDetails.entity).to[ShardTypeNames].futureValue
        unmarshaledGetShardDetails shouldEqual ShardTypeNames(Set(name))
